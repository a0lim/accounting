...
* DB 스키마(Schema) (=논리적 데이터 모델)  
: 선택한 DBMS에 따라 사용자 입장에서 E-R 다이어그램으로 표현된 개념적 구조를 DB에 저장할 형태로 표현한 논리적인 구조  
	- 논리적 데이터 모델링의 결과물  

* 관계 데이터 모델  
	- 일반적으로 많이 사용되는 논리적 데이터 모델  
	- 2차원 테이블 형태인 DB의 논리적 구조  

### 3.2 계층 데이터 모델(hierarchical data model)  
: 트리(tree) 형태인 DB의 논리적 구조  

![4-20](https://user-images.githubusercontent.com/104348646/180137960-f2a20443-e68f-42b8-a36c-c288b223d47a.JPG)
...
*  단점  
	- 복잡한 구조(여러 제약으로 인해 논리적 구조로 자연스럽게 모델링 하기 어려움)  
	- 데이터의 삽입/삭제/수정 등의 연산이나 데이터 검색이 어려움  

### 3.3 네트워크 데이터 모델(network data model)  
: 그래프(graph) 또는 네트워크(network) 형태인 DB의 논리적 구조  

![4-21](https://user-images.githubusercontent.com/104348646/180137968-b7fa27af-9fb4-43eb-8165-1795c4c8d1e7.JPG) 

- 화살표는 일대다 관계만 표현 가능  
	= 다대다 관계를 표현하기 위해 방향이 다른 화살표 두개를 사용  
	
- 오너-멤버 관계(Owner-member relationship)  
: 오너 개체와 멤버 개체가 서로 여러 개와 관계를 맺을 수 있음  

* 장점  
	- 계측 데이터 모델보다 논리적 구조를 더 자연스럽게 모델링
* 단점  
	- 계층 데이터 모델보다 복잡한 구조  
	- 데이터의 삽입/삭제/수정 등의 연산과 데이터 검색이 계층 데이터 모델보다 어려움  

* 객체지향 데이터 모델(object-oriented data model)과 객체관계 데이터 모델(object-relational data model)  
	- 최근에 사용되는 모델  
	- 하지만 관계 데이터 모델의 선호가 더 높음(쉽게 이해 가능하고 연산이 쉬움)
